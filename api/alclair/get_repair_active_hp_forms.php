...
{     
    ...
    //Get Total Records
    $query = "SELECT count(t1.id) FROM repair_form_active_hp AS t1
    ...
    {        
        $query = "SELECT t1.*, to_char(t1.received_date,'MM/dd/yyyy') as received_date, to_char(t1.date_entered,'MM/dd/yyyy') as date_entered, to_char(t1.estimated_ship_date,'MM/dd/yyyy') as estimated_ship_date, to_char(t1.original_ship_date_of_order,'MM/dd/yyyy') as original_ship_date_of_order,
                  ...
                  $query2 = "SELECT t1.*
                  FROM rma_faults_active_hp_log AS t1
...
LEFT JOIN (
    ...
    /*for ($i = 0; $i < $rows_in_result; $i++) {
	 ...
	 $query2 = "SELECT *, to_char(date, 'MM/dd/yyyy    HH24:MI') as date_moved, t2.status_of_repair, t3.first_name, t3.last_name, repair_status_log_active_hp.id AS repair_id
    					FROM repair_status_log_active_hp
    					LEFT JOIN repair_status_table AS t2 ON repair_status_log_active_hp.repair_status_id = t2.order_in_repair
    					LEFT JOIN auth_user AS t3 ON repair_status_log_active_hp.user_id = t3.id
    					WHERE repair_status_log_active_hp.repair_form_id = :repair_form_id
    					ORDER BY date DESC";
