...
*/
	...
	swdApp.controller('Repair_Pipeline_V2', ['$http', '$scope', 'AppDataService', '$upload',  '$cookies', function ($http, $scope, AppDataService, $upload, $cookies) {
	$scope.OrdersList = {};
    $scope.PageIndex = 1;
    $scope.PageSize = window.cfg.PageSize;
    $scope.TotalPages = 0;
    ...
    $scope.LoadData = function () {
		...
		console.log("Monitor ID is " + $scope.monitor_id)
		console.log("Hearing protection is " + $scope.remove_hearing_protection)
        var api_url = window.cfg.apiUrl + "alclair/get_pipeline_repair_2wks.php?PageIndex=" + $scope.PageIndex + "&PageSize=" + $scope.PageSize + "&SearchText=" + $scope.SearchText +"&TODAY_OR_NEXT_WEEK=" + $scope.today_or_next_week + "&RUSH_OR_NOT=" + $scope.rush_or_not + "&REMOVE_HEARING_PROTECTION=" + $scope.remove_hearing_protection + "&monitor_id=" + $scope.monitor_id + "&repair_status_id=" + $scope.repair_status_id;
        //alert(api_url);
        $http.get(api_url)
            .success(function (result) {
