...
$cache_path=$rootScope["RootPath"]."/data/cache/trucking_company_list.txt";
if(file_exists($cache_path))
{
    $viewScope["trucking_company_list"]=file_get_contents($cache_path);
}
else
{
    $stmt = pdo_query( $pdo,
                           'select * from ticket_tracker_truckingcompany order by name',
                            null
                         );	
    $result = pdo_fetch_all($stmt,PDO::FETCH_ASSOC);
    $viewScope["trucking_company_list"]=json_encode($result);
    $cache_content=$viewScope["trucking_company_list"];
    file_put_contents($cache_path,$cache_content);
}
