...
$query5 =  "SELECT  start_filter, end_filter, batch_num, disposal_well_id FROM recon_report ORDER BY batch_num desc limit 1";
$stmt5 = pdo_query( $pdo, $query5, null); 
$result5 = pdo_fetch_all( $stmt5 );

foreach($result5 as $ForLoop) {
	if($ForLoop["start_filter"] != $_REQUEST["startdate"] || $ForLoop["end_filter"] != $_REQUEST["enddate"] || $ForLoop["disposal_well_id"] != $_REQUEST["SearchDisposalWell"])
//if($ForLoop["batch_num"] != 1222)
	{
...
$file_=$rootScope["RootPath"]."data/exportpdf/".$filename;
 ...
 // HERE IS WHERE THE EMAIL SECTION OF THE CODE STARTS
    ...
    //$user_email.=",tyler@alclair.com,tyler.folsom@gmail.com";
    $user_email.=",dave@baroverhat.com,dedwards@baroverhat.com,tcoombs@baroverhat.com,jbetts@baroverhat.com"; 
...
//{
	...
	$body="<p>The attached file is the Reconciliation Report between {$_REQUEST["startdate"]} and {$_REQUEST["enddate"]}.</p><p>This e-mail has been sent to David Edwards and Traci Coombs.</p>";
	$mail->MsgHTML($body);
...
// ELSE STATEMENT FOR EMAILING THE PDF IF THE USER HAD ALREADY PRODUCED THE PDF AND CONFIRMED IT WAS CORRECT
// BELOW IS JUST THE EMAIL ROUTINE
// ABOVE IS THE PDF CREATION AND THE EMAIL ROUTINE
else
{
	
	$filename = "Recon-Report-from {$ForLoop["start_filter"]} to {$ForLoop["end_filter"]} as Batch No. {$ForLoop["batch_num"]}.pdf";
    //$pdf->Output("../../data/exportpdf/$filename", "F"); 
...
//$list=json_decode($json,true);
 ...
 // HERE IS WHERE THE EMAIL SECTION OF THE CODE STARTS
 	//$user_email = array();
    //array_push($user_email,$_SESSION["Email"]);
    $user_email = $_SESSION["Email"];
    //$user_email.=",tyler@alclair.com,tyler.folsom@gmail.com";
    $user_email.=",dedwards@baroverhat.com,tcoombs@baroverhat.com,tjohnston@baroverhat.com";
//if(file_exists($pdf)&&!empty($user_email))
//{
	$mail= new PHPMailer();
    $mail->IsSendmail(); // telling the class to use IsSendmail
