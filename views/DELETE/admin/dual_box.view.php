...
<html>
	...
	<body>
		<table>
			<tr>
				<th colspan="2"><h1>Available Source Wells</h1></th>
				<th colspan="2"><h1>Selected Source Wells</h1></th>
  			</tr>
		</table>
    ...
    <div class="col-xs-5">
        <select name="from[]" id="search" class="form-control" size="20" multiple="multiple">
	        <?php
				...
				$query='select t1.id as id, t1.file_number as file_number, t1.current_well_name as name, t2.name as operator_name from wells_list as t1 
		        ...
		        for ($i=0; $i<sizeof($row); $i++) {
						$a .= "<option ";
						$a .='value="'.$row[$i]['id'].'">';
						$a .= $row[$i]['file_number']. '&nbsp;&nbsp;'.$row[$i]['name']. '&nbsp;&nbsp;|&nbsp;&nbsp;'.$row[$i]['operator_name'];
						//. '&nbsp;&nbsp;|&nbsp;&nbsp;'.$row2[$j]['operator_name']
						$a .= '</option>';
				}
						echo $a;
				?>
    ...
    <div class="col-xs-5">
        <select name="to[]" id="search_to" class="form-control" size="20" multiple="multiple">
	       	<?php    
				...
				$query2='select t1.id as id, t1.file_number as file_number, t1.current_well_name as name, t2.name as operator_name from ticket_tracker_well as t1 
				LEFT JOIN ticket_tracker_operator as t2 on t1.current_operator_id = t2.id
				where 1=1';
		        ...
		        for ($j=0; $j<sizeof($row2); $j++) {
						$b .= "<option ";
						$b .='value="'.$row2[$j]['id'].'">';
						$b .= $row2[$j]['file_number']. '&nbsp;&nbsp;'.$row2[$j]['name']. '&nbsp;&nbsp;|&nbsp;&nbsp;'.$row2[$j]['operator_name'];
						$b .= '</option>';
				}
						echo $b;
				?>
...
$(document).ready(function() {
    $('#search').multiselect({
        search: {
            left: '<input type="text" name="q" class="form-control" placeholder="Search by well name or by operator" />',
            right: '<input type="text" name="q" class="form-control" placeholder="Search by well name or by operator" />',
        }
    });
});
