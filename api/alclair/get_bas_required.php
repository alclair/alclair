...
{     
    ...
    if ($_REQUEST['monitor_id'] > 0 ) {
		...
		if( $_REQUEST['TODAY_OR_NEXT_WEEK'] == '1') {   // MEANS TODAY
			...
			$conditionSql.=" and (t1.estimated_ship_date = :Date) AND (t1.order_status_id <= 5 AND t1.order_status_id >=1) ";
			...
			$conditionSql.=" and (t1.estimated_ship_date > :Today) AND (t1.estimated_ship_date <= :Date) AND (t1.order_status_id <= 5 AND t1.order_status_id >=1) ";
