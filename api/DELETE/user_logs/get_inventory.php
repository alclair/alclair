...
{     
    ...
    $query = "SELECT id as i_d FROM inventory WHERE active = 'TRUE' $conditionSql ";
    ...
    //  IF THERE IS AN ACTIVE INVENTORY LOG OPEN
    ...
    else   //  IF THERE IS  NOT AN ACTIVE INVENTORY LOG OPEN
    {
	    	$stmt = pdo_query( $pdo, 
						...
						array(':created_by_id'=>$_SESSION['UserId'], ':active'=>'TRUE', ':type_id'=>2) 	
	    										);	    										
		...
		$params[":created_by_id"]=$_SESSION["UserId"];
		//$params[":created_by_id"]=2;
		$query = "SELECT * FROM inventory WHERE active = 'TRUE' $conditionSql ";
		$stmt = pdo_query( $pdo, $query, $params);
		$result = pdo_fetch_all( $stmt );
