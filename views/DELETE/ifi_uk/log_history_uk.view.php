...
<div ng-controller="ifi_LogHistory">
    ...
    <div class="press-enter" style="width:99%;margin-left:10px;">
        <div class="row">
            <div class="col-md-3">
                <a href="<?=$rootScope['RootURL']?>/ifi_uk/orders_uk"><b style="font-size:20px">Movement Log</a> (Total: {{TotalRecords}})</b>&nbsp;&nbsp;<b style="display:inline; font-size:20px; color: #007FFF"> </b>
            </div>
		...
		<table>		
			...
			<tbody>
				<tr ng-repeat='order in OrdersList'>
					...
					<td  ng-if="order.movement=='EXPORT'" style="text-align:center;" data-title="Movement"><a href="<?=$rootScope['RootUrl']?>/ifi_uk/edit_export_uk/{{order.id}}">{{order.movement}}</td>					
					<td  style="text-align:center;" data-title="Type">{{order.movement_type}}</td>	
					<td  style="text-align:center;" data-title="ID"><a href="<?=$rootScope['RootUrl']?>/ifi_uk/serial_numbers_in_log/{{order.id}}">{{order.id}}</a></td>			
