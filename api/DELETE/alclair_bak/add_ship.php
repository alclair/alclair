...
if($start_cart["barcode"][0] == 'R') {
	...
	$query = pdo_query($pdo, "SELECT * FROM repair_status_table WHERE status_of_order = 'Done'", null);
	...
	if($result[0]["id_of_repair"] === null || $result[0]["id_of_repair"] < 1) { // SOME LINES FROM qc_form WON'T HAVE AN id_of_order BECAUSE THE ORIGINAL PROGRAMMING 	DIDN'T INCLUDE THAT COLUMN
		...
		$stmt = pdo_query( $pdo, "INSERT INTO repair_status_log (date, repair_form_id, repair_status_id, notes,  user_id) VALUES (now(), :repair_form_id, :status_id, :notes, :user_id) RETURNING id", array(':repair_form_id'=>$response['ID_is'], ':status_id'=>$status_id, ':notes'=>"Moved to done by shipping", ':user_id'=>$_SESSION['UserId']));					 					 
