...
{	  
    ...
    $query = "SELECT to_char(t1.qc_date,'dd') AS created, initial_pass_or_fail, ( SELECT COUNT(initial_pass_or_fail) ) AS num_status
              FROM qc_form as t1
              WHERE to_char(qc_date,'yyyy') = '$year' AND to_char(qc_date,'MM') = '$month' $condition AND build_type_id = 1 AND (initial_pass_or_fail = 'PASS' OR initial_pass_or_fail = 'FAIL')
              GROUP BY created, initial_pass_or_fail
