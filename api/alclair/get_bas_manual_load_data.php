...
{     
    ...
    $IEMs = [];
    $IEMs["versa"] = $_REQUEST["versa"];
    $IEMs["dual_xb"] = $_REQUEST["dual_xb"];
    $IEMs["st3"] = $_REQUEST["st3"];
    $IEMs["tour"] = $_REQUEST["tour"];
    $IEMs["rsm"] = $_REQUEST["rsm"];
    $IEMs["cmvk"] = $_REQUEST["cmvk"];
    $IEMs["spire"] = $_REQUEST["spire"];
	$IEMs["studio4"] = $_REQUEST["studio4"];
	$IEMs["revx"] = $_REQUEST["revx"];
	$IEMs["electro"] = $_REQUEST["electro"];
	$IEMs["esm"] = $_REQUEST["esm"];
	$IEMs["dkm"] = $_REQUEST["dkm"];
	$IEMs["icon"] = $_REQUEST["icon"];
	
	$IEMs = [];
    $IEMs[0]["model"] = "Versa";
    $IEMs[0]["count"] = $_REQUEST["versa"];
    $IEMs[1]["model"] = "Dual XB";
    $IEMs[1]["count"] = $_REQUEST["dual_xb"];
    $IEMs[2]["model"] = "ST3";
    $IEMs[2]["count"] = $_REQUEST["st3"];
    $IEMs[3]["model"] = "Tour";
    $IEMs[3]["count"] = $_REQUEST["tour"];
    $IEMs[4]["model"] = "RSM";
	$IEMs[4]["count"] = $_REQUEST["rsm"];
    $IEMs[5]["model"] = "CMVK";
    $IEMs[5]["count"] = $_REQUEST["cmvk"];
    $IEMs[6]["model"] = "Spire";
    $IEMs[6]["count"] = $_REQUEST["spire"];
    $IEMs[7]["model"] = "Studio4";
    $IEMs[7]["count"] = $_REQUEST["studio4"];
    $IEMs[8]["model"] = "Rev X";
    $IEMs[8]["count"] = $_REQUEST["revx"];
    $IEMs[9]["model"] = "Electro";
    $IEMs[9]["count"] = $_REQUEST["electro"];
    $IEMs[10]["model"] = "ESM";
    $IEMs[10]["count"] = $_REQUEST["esm"];    
    $IEMs[11]["model"] = "DKM";
    $IEMs[11]["count"] = $_REQUEST["dkm"];
    $IEMs[12]["model"] = "ICON";
    $IEMs[12]["count"] = $_REQUEST["icon"];
...
for ($i = 0; $i < count($IEMs); $i++) {
	 	$stmt= pdo_query($pdo,  "SELECT id FROM monitors WHERE name = :name", array(":name"=>$IEMs[$i]["model"]));
	 	$monitor_id = pdo_fetch_all( $stmt);
	 	
	 	$stmt = pdo_query( $pdo, "SELECT * FROM parts_required AS t1 LEFT JOIN part_table AS t2 ON t1.part_id = t2.id WHERE monitor_id = :monitor_id", array(":monitor_id"=>$monitor_id[0]["id"])); 
	 	$get_parts = pdo_fetch_all( $stmt);
	 	
	 	for ($j = 0; $j < count($get_parts); $j++) {
		     if($get_parts[$j]["part_id"] == 1) {
			     $part_2389_casing = $get_parts[$j]["quantity"] * $IEMs[$i]["count"]  + $part_2389_casing;
		     } elseif($get_parts[$j]["part_id"] == 2) {
			     $part_2015_casing = $get_parts[$j]["quantity"] * $IEMs[$i]["count"] + $part_2015_casing;
		     } elseif($get_parts[$j]["part_id"] == 3) {
			     $part_2091_casing = $get_parts[$j]["quantity"] * $IEMs[$i]["count"] + $part_2091_casing;
		     } elseif($get_parts[$j]["part_id"] == 4) {
			     $part_33A007_casing = $get_parts[$j]["quantity"] * $IEMs[$i]["count"] + $part_33A007_casing;
		     } elseif($get_parts[$j]["part_id"] == 5) {
			     $part_3800_casing = $get_parts[$j]["quantity"] * $IEMs[$i]["count"] + $part_3800_casing;
		     } elseif($get_parts[$j]["part_id"] == 6) {
			      $part_2800_casing = $get_parts[$j]["quantity"] * $IEMs[$i]["count"] + $part_2800_casing;
		     } elseif($get_parts[$j]["part_id"] == 7) {
			     $part_E25_casing = $get_parts[$j]["quantity"] * $IEMs[$i]["count"] + $part_E25_casing;
		     } elseif($get_parts[$j]["part_id"] == 8) {
			     $part_35A007_casing = $get_parts[$j]["quantity"] * $IEMs[$i]["count"] + $part_35A007_casing;
		     } elseif($get_parts[$j]["part_id"] == 9) {
			     $part_36A007_casing = $get_parts[$j]["quantity"] * $IEMs[$i]["count"] + $part_36A007_casing;
		     } elseif($get_parts[$j]["part_id"] == 10) {
			     $part_17A003_casing = $get_parts[$j]["quantity"] * $IEMs[$i]["count"] + $part_17A003_casing;
		     } elseif($get_parts[$j]["part_id"] == 11) {
			     $part_6500_casing = $get_parts[$j]["quantity"] * $IEMs[$i]["count"] + $part_6500_casing;
		     } elseif($get_parts[$j]["part_id"] == 12) {
			     $part_1723WT03_9_casing = $get_parts[$j]["quantity"] * $IEMs[$i]["count"] + $part_1723WT03_9_casing;
		     } elseif($get_parts[$j]["part_id"] == 13) {
			     $part_3800_filtered_casing = $get_parts[$j]["quantity"] * $IEMs[$i]["count"] + $part_3800_filtered_casing;
		     } 
	 	}	 	
	}
	$response["test"] = "IEM is  " . $IEMs[$i]["model"];
	//echo json_encode($response);
	//exit;	 
