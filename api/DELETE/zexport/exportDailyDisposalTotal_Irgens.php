...
{	  
    ...
    $query =   "select t1.id, t1.ticket_number, to_char(t1.date_delivered,'yyyy-MM-dd') as date_delivered, t1.barrels_delivered, twatertype.type as water_type_name,t1.barrel_rate, t1.notes,
                ...
                where date_delivered >= to_date('$startdate','YYYY-MM-DD') and date_delivered <= to_date('$enddate','YYYY-MM-DD')  and t1.disposal_well_id = 14   and t1.trucking_company_id = 48          
    ...
    foreach($result as $item)
    {
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue("B$rowcount","Irgens") // Hard code to show Irgens disposal well
    ...
    $filename = "Export-Disposals-From-Irgens-During(".str_replace("/","-",$startdate).")-To(".str_replace("/","-",$enddate).")-".time().".xlsx";
    //$filename = "hello.xlsx";
