...
if($_REQUEST["id"][0] == 'R') {
	...
	if($result2[0]["id_of_qc_form"] === null || $result2[0]["id_of_qc_form"] < 1) { // SOME LINES FROM import_orders WON'T HAVE AN id_of_qc_form BECAUSE THE ORIGINAL PROGRAMMING DIDN'T INCLUDE THAT COLUMN
		...
		// IF STATEMENT  - ONLY FOR ACTIVE HP AND AFTER NOVEMBER 1ST, 2022
		if( stristr( $result2[0]["model"], "Exp") || !stristr( $result2[0]["model"], "EXP") && $result2[0]["date"] > "10/31/2022 23:59:59" ) {
			$stmt = pdo_query($pdo, "SELECT * FROM qc_form_active_hp WHERE customer_name = :customer_name AND order_id = :order_id", array(":customer_name"=>$_REQUEST["customer_name"], ":order_id"=>$result2["order_id"]));
			$result3 = pdo_fetch_all($stmt);
		...
		} else { // ALL NON-ACTIVE HP MONITORS AND BEFORE NOVEMBER 1ST, 2022
