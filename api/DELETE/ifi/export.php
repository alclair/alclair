...
{	   
	...
	for ($i = 0; $i < $num_of_sn; $i++) {
		$stmt = pdo_query( $pdo, 'SELECT * FROM serial_numbers WHERE serial_number = :serial_number', array(":serial_number"=>$serial_numbers[$i]));	
...
for ($i = 0; $i < $num_of_sn; $i++) {
	...
	$query = "UPDATE serial_numbers SET status = 'SHIPPED' WHERE serial_number = :serial_number";
	...
	$stmt = pdo_query( $pdo, "INSERT INTO order_tracking (serial_number, date, status, discount, notes, log_id) VALUES (:serial_number, now(), :status, :discount, :notes, :log_id) RETURNING id",
