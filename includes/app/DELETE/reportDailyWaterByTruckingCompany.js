swdApp.controller('reportDailyWaterByTruckingCompany', ['$http', '$scope', 'AppDataService', function ($http, $scope, AppDataService) {
    ...
    $scope.loadChart = function () {
        myblockui();
        var api_url = window.cfg.apiUrl + "reports/daily_water_by_trucking_company.php?year=" + $scope.year_month + "&month=" + $scope.month_month + "&disposalwell=" + $scope.disposalwell;
        $http.get(api_url).success(function (result) {
            ...
            d3DarwStackGroup(layers, "div_daily_water_by_trucking_company", "daily_water_by_trucking_copany_grouped", "daily_water_by_trucking_copany_stacked", labels, { isgroup: false, category20: true });
        }).error(function () {
            $.unblockUI();
    ...
    $scope.selectMonth = function () {
        $("#div_daily_water_by_trucking_company").html("");
        $scope.loadChart();
