...
if($start_cart["barcode"][0] == 'R' || $start_cart["barcode"][0] == '9') {
	...
	$query = pdo_query($pdo, "SELECT * FROM repair_status_table WHERE status_of_repair = 'Artwork'", null);
...
$query = pdo_query($pdo, "SELECT * FROM order_status_table WHERE status_of_order = 'Artwork'", null);
$result = pdo_fetch_array($query);
