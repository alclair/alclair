   ...
   */           
	...
	for($j=0; $j<count($num_of_fails_in_day); $j++) {   
	    ...
	    for($i=0; $i<count($result2); $i++) {
				...
				$query = "SELECT id, sound_signature, sound_balanced, sound_correct_model, to_char(qc_date,'dd') AS created FROM qc_form WHERE id = :id";
				$stmt = pdo_query( $pdo, $query, array(":id"=>$result2[$i]["id"])); 
				$result_sound = pdo_fetch_array( $stmt );
				if(!$result_sound["sound_signature"]  || !$result_sound["sound_balanced"] || !$result_sound["sound_correct_model"] ) {
					$count_sound = $count_sound + 1;
				}
				
				$query = "SELECT id, shells_defects, shells_colors, shells_faced_down, shells_label, shells_edges, shells_shine, shells_canal, shells_density, to_char(qc_date,'dd') AS created FROM qc_form WHERE id = :id";
				$stmt = pdo_query( $pdo, $query, array(":id"=>$result2[$i]["id"])); 
				$result_shells = pdo_fetch_array( $stmt );
				if(!$result_shells["shells_defects"]  || !$result_shells["shells_colors"] || !$result_shells["shells_faced_down"] || !$result_shells["shells_label"] || !$result_shells["shells_edges"] || !$result_shells["shells_shine"] || !$result_shells["shells_canal"] || !$result_shells["shells_density"]) {
					$count_shells = $count_shells + 1;
				}
				
				$query = "SELECT id, faceplate_seams, faceplate_shine, faceplate_colors, faceplate_rounded, faceplate_foggy, faceplate_residue, to_char(qc_date,'dd') AS created FROM qc_form WHERE id = :id";
				$stmt = pdo_query( $pdo, $query, array(":id"=>$result2[$i]["id"])); 
				$result_faceplate = pdo_fetch_array( $stmt );
				if(!$result_faceplate["faceplate_seams"]  || !$result_faceplate["faceplate_shine"] || !$result_faceplate["faceplate_colors"] || !$result_faceplate["faceplate_rounded"]  || !$result_faceplate["faceplate_foggy"]  || !$result_faceplate["faceplate_residue"]) {
				...
				$query = "SELECT id, ports_cleaned, ports_smooth, ports_glued_correctly, ports_kinked_tube, ports_crushed_damper, to_char(qc_date,'dd') AS created FROM qc_form WHERE id = :id";
				$stmt = pdo_query( $pdo, $query, array(":id"=>$result2[$i]["id"])); 
				$result_ports = pdo_fetch_array( $stmt );
				if(!$result_ports["ports_cleaned"] || !$result_ports["ports_smooth"] || !$result_ports["ports_glued_correctly"] || !$result_ports["ports_kinked_tube"] || !$result_ports["ports_crushed_damper"]) {
				...
				if($result_artwork["sound_signature"] && $result_artwork["sound_balanced"] && $result_artwork["sound_correct_model"] && 
						$result_artwork["shells_defects"] && $result_artwork["shells_colors"]&& $result_artwork["shells_faced_down"] && 														$result_artwork["shells_label"]&& $result_artwork["shells_edges"] && $result_artwork["shells_shine"] && $result_artwork["shells_canal"] && $result_artwork["shells_density"] && $result_artwork["faceplate_seams"] && $result_artwork["faceplate_shine"] && $result_artwork["faceplate_colors"] && 												$result_artwork["faceplate_rounded"] && $result_artwork["faceplate_foggy"] && $result_artwork["faceplate_residue"] &&
						$result_artwork["jacks_location"] && $result_artwork["jacks_debris"] && $result_artwork["jacks_cable"] &&
						$result_artwork["ports_cleaned"] && $result_artwork["ports_smooth"] && $result_artwork["ports_glued_correctly"] && $result_artwork["ports_kinked_tube"]&& $result_artwork["ports_crushed_damper"]) {
