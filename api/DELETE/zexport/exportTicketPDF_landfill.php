...
{	             
    ...
    if(empty($id) == false)
    {
        $query =  "select t1.*, to_char(t1.date_delivered,'MM/dd/yyyy') as date_delivered, 
                  to_char(t1.date_created,'MM/dd/yyyy') as date_created, t2.freight_fee, t2.tipping_fee, t2.name
    ...
    if(count($result) > 0)
    {
        ...
        {
	        $fees = $ticket["freight_fee"] + $ticket["tipping_fee"];
            //$pdf->lastPage();
            $pdf->AddPage();
...
<table>
    ...
    <tr>
        <td>{$ticket["light_weight"]}</td>
        <td>{$ticket["heavy_weight"]}</td>
    </tr>
    <tr><td colspan=\"2\" style='height:$td_height;'></td></tr>
    <tr style=\"font-weight:bold;\">
        <td>Tons:</td>
    ...
    <tr>
        <td>{$ticket["tons"]}</td>
        <td>{$ticket["picocuries"]}</td>
    </tr>
    
    <tr><td colspan=\"2\" style='height:$td_height;'></td></tr>
    <tr style=\"font-weight:bold;\">
        <td>Landfill Disposal Site:</td>
    </tr>
    <tr>
        <td>{$ticket["name"]}</td>
    </tr>
   ...
   <tr><td colspan=\"2\" style='height:$td_height;'></td></tr>
    ...
    <tr><td colspan=\"2\" style='height:$td_height;'></td></tr>
            ...
            $stmt_file = pdo_query($pdo, "select filepath from ticket_tracker_indexupload_landfill where ticket_id = {$ticket["id"]}",null);
