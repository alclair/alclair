...
{     
	...
	if($_REQUEST["company_id"] >= 1)
    {
		...
		$query = "SELECT * FROM shipping_addresses WHERE id = :id"; 
		$params2[":id"] = $_REQUEST["company_id"]; 
		$stmt = pdo_query( $pdo, $query, $params2 );
		$row = pdo_fetch_array( $stmt );
		$ship_to_name = $row['title'];
		
		//$response['message'] = 'Did not work ' . $ship_to_name . ' END';
		//echo json_encode($response);
		//exit;


		$conditionSql .= " AND (t1.title = :company)";
		$params[":company"] = $ship_to_name;
	}
    ...
    //Get Total Records
    $query = "SELECT count(t1.id) FROM log_movement_uk AS t1
