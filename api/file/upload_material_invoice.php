...
{
    $time = time();
	$query="SELECT invoice_number FROM materials_tracker WHERE id=:id";
	$stmt=pdo_query($pdo,$query,array(":id"=>$_REQUEST["id"]));
	$row=pdo_fetch_array($stmt);
	$tmp=explode(".",$_FILES['documentfile']['name']);
	$ext=$tmp[count($tmp)-1];
	$file_name=$row["invoice_number"]."_".$time.".".$ext;
    $path = "../../data/".$file_name;
    ...
    {   
        ...
        $stmt = pdo_query(
                            $pdo,
                            "INSERT INTO materials_invoice_indexupload
                            ( filepath, date_uploaded,  uploaded_by_id,
                            invoice_id)
                            VALUES
                            ( :filepath, now(), ".$_SESSION["UserId"].",
                            :invoice_id);", 
                            array( ":filepath" => $file_name, ":invoice_id" => $_REQUEST['id'] )
                         );
