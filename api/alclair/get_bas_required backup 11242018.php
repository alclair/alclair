...
{     
    ...
    if ($_REQUEST['monitor_id'] > 0 ) {
		...
		if( $_REQUEST['TODAY_OR_NEXT_WEEK'] == '1') {   // MEANS TODAY
			$conditionSql.=" and (t1.estimated_ship_date = :Date) AND t1.order_status_id != 12AND t1.order_status_id != 18 ";
			$params[":Date"]=  $today_4_sql;
		} elseif ($_REQUEST['TODAY_OR_NEXT_WEEK'] == '0') { // MEANS PAST DUE
			$conditionSql.=" and (t1.estimated_ship_date < :Date) AND t1.order_status_id != 12 AND t1.order_status_id != 18";
			$params[":Date"]= $today_4_sql;	
			...
			$conditionSql.=" and (t1.estimated_ship_date > :Today) AND (t1.estimated_ship_date <= :Date) AND t1.order_status_id != 12 AND t1.order_status_id != 18";
