...
<div ng-controller="Repairs_Done_By_Date">
    <!-- Main Container Starts -->
    <div class="press-enter" style="width:99%;margin-left:10px;">
        <div class="row">
            <div class="col-md-4">
                <a href="<?=$rootScope['RootURL']?>/alclair/repair_list"><b style="font-size:20px">Repairs</a> (Total: {{TotalRecords}})</b>&nbsp;&nbsp;<b style="display:inline; font-size:20px; color: #007FFF"> </b><span style="color: #FF0000">  {{traveler.rma_number}}</span></a>
            </div>
            ...
            <div class="col-md-3">
                <input type="checkbox" ng-model="rush_or_not" ng-true-value="1" ng-false-value="0" style="margin-top:16px"> &nbsp; <b style="font-size:14px;color:gray">RUSH REPAIRS ONLY</b><br />
            </div>
		...
		<div class="row alert" style='background-color:#ddd;'>
            ...
            <div class="col-sm-2">
                    <div class="input-group">
						<input type="text"  ng-model="SearchText" placeholder="Name or rma #"  uib-typeahead="customer as customer.customer_name for customer in customers| filter:{customer_name:$viewValue}| limitTo:8"  typeahead-on-select="SearchText=$item.customer_name" typeahead-editable="true" class="form-control" >		
		...
		<table>		
			...
			<tbody>
				<tr ng-repeat='repair in RepairsList'>
					<td  style="text-align:center;" data-title="Customer Name"><a href="<?=$rootScope['RootUrl']?>/alclair/edit_repair_form/{{repair.id}}">{{repair.customer_name}} </a></td>
					
					<td  style="text-align:center;" data-title="Repair Status">{{repair.status_of_repair}}</td>
					
					<td  style="text-align:center;" data-title="RMA #">{{repair.rma_number}}</td>
					<td  style="text-align:center;" data-title="Completed Date">{{repair.date_done}}</td>
					<td  style="text-align:center;" data-title="Model">{{repair.model}}</td>
					<td  style="text-align:center;" data-title="# Done">{{repair.num_done}}</td>					
										
                    <td data-title="Options">
	                    <div style="text-align:center;" >  
		                    
		                    <!-- ng-disabled="order.highrise != 1" -->
		                    &nbsp;&nbsp;<button  type="button" class="btn btn-primary btn-xs" ng-click="PDF(repair.id);">Traveler</button>					
						<?php if($_SESSION["UserName"] == 'Scott' || $_SESSION["UserName"] == 'admin'  || $_SESSION["UserName"] == 'Amanda' || $_SESSION["UserName"] == 'Will') { ?>
							...
							&nbsp;&nbsp;<button  type="button" class="btn btn-primary btn-xs" ng-click="LoadSelectDateModal(repair.id);">DONE</button>		
						<?php } ?>
