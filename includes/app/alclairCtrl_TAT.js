swdApp.controller('TAT', ['$http', '$scope', 'AppDataService', '$upload',  '$cookies', function ($http, $scope, AppDataService, $upload, $cookies) {
	$scope.OrdersList = {};
    $scope.PageIndex = 1;
    ...
    $scope.LoadData = function () {
		...
		console.log("Date is " + $scope.SearchStartDate)
		//$scope.SearchStartDate = '3/5/2018';
		
        var api_url = window.cfg.apiUrl + "alclair/get_turn_around_time.php?PageIndex=" + $scope.PageIndex + "&PageSize=" + $scope.PageSize + "&StartDate="+moment($scope.SearchStartDate).format("MM/DD/YYYY")+"&EndDate="+moment($scope.SearchEndDate).format("MM/DD/YYYY");
        //alert(api_url);
        $http.get(api_url)
            .success(function (result) {
	            console.log("Test1 is " + JSON.stringify(result.test1))
	            console.log("Test2 is " + JSON.stringify(result.test2))
	            //console.log("Test3 is " + JSON.stringify(result.test3))
	            console.log("Counter is " + (result.test4))
	             
	             console.log("Min is " + result.min + " Max is " + result.max + " Avg is " + result.avg + " Total is " + result.num_of_orders1 + " and " + result.num_of_orders2 + " median is " + result.median)
	            
	            if(result.min == 0 ) {
		            $scope.there_is_a_null = 1;
	            } else {
		            $scope.there_is_a_null = 0;
	            }
	            ...
	            $scope.total_orders = result.num_of_orders1;
	            //console.log("Data is " + JSON.stringify(result.data)))
                $scope.OrdersList = result.data;
                $scope.OrdersList_zero = result.data_zero;
                //console.log("Name is " + $scope.OrdersList[0].designed_for)
    ...
    $scope.Excel = function () {
	   myblockui();
	   	...
	   	var api_url = window.cfg.apiUrl + "export/alclair_excel_export_tat.php?&StartDate=" + moment($scope.SearchStartDate).format("MM/DD/YYYY") + "&EndDate=" + moment($scope.SearchEndDate).format("MM/DD/YYYY")
...
swdApp.controller('TAT_Repair', ['$http', '$scope', 'AppDataService', '$upload',  '$cookies', function ($http, $scope, AppDataService, $upload, $cookies) {
	$scope.OrdersList = {};
    $scope.PageIndex = 1;
    $scope.PageSize = window.cfg.PageSize;
    $scope.TotalPages = 0;
    ...
    $scope.LoadData = function () {
        ...
        var api_url = window.cfg.apiUrl + "alclair/get_turn_around_time_repairs.php?PageIndex=" + $scope.PageIndex + "&PageSize=" + $scope.PageSize + "&StartDate="+moment($scope.SearchStartDate).format("MM/DD/YYYY")+"&EndDate="+moment($scope.SearchEndDate).format("MM/DD/YYYY");
        //alert(api_url);
        $http.get(api_url)
            .success(function (result) {
	            console.log("Test1 is " + JSON.stringify(result.test1))
	            ...
	            console.log("Counter is " + (result.test4))
	             ...
	             console.log("Min is " + result.min + " Max is " + result.max + " Avg is " + result.avg + " Total is " + result.num_of_orders1 + " and " + result.num_of_repairs2 + " median is " + result.median)
                ...
                $scope.RepairsList_zero = result.data_zero;
                //console.log("Name is " + $scope.OrdersList[0].designed_for)
	...
	swdApp.controller('Pipeline', ['$http', '$scope', 'AppDataService', '$upload',  '$cookies', function ($http, $scope, AppDataService, $upload, $cookies) {
	$scope.OrdersList = {};
    $scope.PageIndex = 1;
    $scope.PageSize = window.cfg.PageSize;
    $scope.TotalPages = 0;
	...
	swdApp.controller('BAs_Required', ['$http', '$scope', 'AppDataService', '$upload',  '$cookies', function ($http, $scope, AppDataService, $upload, $cookies) {
	$scope.OrdersList = {};
    $scope.PageIndex = 1;
    $scope.PageSize = window.cfg.PageSize;
    $scope.TotalPages = 0;
    ...
    $scope.init = function () {
	    ...
	    // FOR LOOP IS HERE TO REMOVE THE FIRST TWO ENTRIES IN PRINTED_OR_NOT
	    // THE FIRST TWO ENTRIES ARE "PAST DUE" AND "TODAY"
	    for(i=2; i < AppDataService.TODAY_OR_NEXT_WEEK.length; i++) {
			$scope.TODAY_OR_NEXT_WEEK[i-2] = AppDataService.TODAY_OR_NEXT_WEEK[i];    
	    }
        //$scope.SearchText = $cookies.get("SearchText");
        //$scope.cust_name = $cookies.get("SearchText");
        //$scope.qc_form.cust_name = $cookies.get("SearchText");
	...
	swdApp.controller('Repair_Pipeline', ['$http', '$scope', 'AppDataService', '$upload',  '$cookies', function ($http, $scope, AppDataService, $upload, $cookies) {
	$scope.OrdersList = {};
    $scope.PageIndex = 1;
    $scope.PageSize = window.cfg.PageSize;
    $scope.TotalPages = 0;
	...
	swdApp.controller('TAT_hearing_protection', ['$http', '$scope', 'AppDataService', '$upload',  '$cookies', function ($http, $scope, AppDataService, $upload, $cookies) {
	$scope.OrdersList = {};
    $scope.PageIndex = 1;
    $scope.PageSize = window.cfg.PageSize;
    $scope.TotalPages = 0;
    ...
    $scope.LoadData = function () {
        ...
        var api_url = window.cfg.apiUrl + "alclair/get_turn_around_time_hp.php?PageIndex=" + $scope.PageIndex + "&PageSize=" + $scope.PageSize + "&StartDate="+moment($scope.SearchStartDate).format("MM/DD/YYYY")+"&EndDate="+moment($scope.SearchEndDate).format("MM/DD/YYYY");
        //alert(api_url);
        $http.get(api_url)
            .success(function (result) {
	            console.log("Test1 is " + JSON.stringify(result.test1))
