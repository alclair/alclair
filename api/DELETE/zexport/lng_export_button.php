...
$url=$rootScope["RootUrl"]."/api/reports/lng_export_data.php?lng_queens=$lng_queens&StartDate=$startdate&EndDate=$enddate&UserId=$UserId&IsAdmin=$IsAdmin&IsManager=$IsManager&token=".$rootScope["SWDApiToken"];
...
if(file_exists($file_lng)) 
{
	...
	$body3="<p>The attached file is an Excel document displaying the data from the plots window.</p>";
	$mail3->MsgHTML($body3);
	//$mail3->AddAttachment($file_lng, "ZzZzZ-".date("m-d-Y").".xlsx");
	//$mail3->AddAttachment($file_lng, "Step4.xlsx");
	$mail3->AddAttachment($file_lng, "Export-Queen-Data-".date("m-d-Y").".xlsx");
	//$mail->AddAttachment($file_disposal, "DailyDisposal.xlsx");
	$response['data2'] = 'In here';
