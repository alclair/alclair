...
{     
    ...
    if(!empty($_REQUEST["SearchText"]))
    {
        ...
        $conditionSql .= " and (t1.ticket_number ilike :SearchText or twell.current_well_name ilike :SearchText)";
    ...
    {        
        $query = "select t1.*, to_char(t1.date_delivered,'MM/dd/yyyy') as date_delivered, t1.barrel_rate, 
                  to_char(t1.date_created,'MM/dd/yyyy') as date_created,  twell.file_number as source_well_file_number, twell.current_well_name as source_well_name, toperator.name as source_well_operator_name, toperator.id as operator_id,
                  t2.type as water_type_name, t3.common_name as disposal_well_name, t4.name as company_name, t5.first_name as first_name, t5.last_name as last_name
                  ...
                  left join auth_user as t5 on t1.created_by_id = t5.id 
                  where t1.id = :id";
    }
    else
    {
        $query = "select t1.id, t1.ticket_number, to_char(t1.date_delivered,'MM/dd/yyyy') as date_delivered, t1.barrels_delivered, twatertype.type as water_type_name,t1.barrel_rate, t1.created_by_id, 
        twell.file_number as source_well_file_number, twell.current_well_name as source_well_name, toperator.name as source_well_operator_name,
			t3.common_name as disposal_well_name, toperator.id as operator_id, t5.id as users_id,  t5.is_superuser as is_admin
                  from ticket_tracker_ticket as t1
				  left join ticket_tracker_disposalwell as t3 on t1.disposal_well_id = t3.id
                  left join ticket_tracker_well as twell on t1.source_well_id =twell.id
                  left join ticket_tracker_operator toperator on twell.current_operator_id=toperator.id
                  left join ticket_tracker_watertype as twatertype on t1.water_type_id = twatertype.id
                  left join auth_user as t5 on :session_userid = t5.id
