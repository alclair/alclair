...
{	             
    $id = $_REQUEST["id"];
    $html = "<div>Haven't found any tickets.</div>";  
    $result = array();

    if(empty($id) == false)
    {
        $query =  "select t1.*, to_char(t1.date_delivered,'MM/dd/yyyy') as date_delivered, t1.barrel_rate, 
                  to_char(t1.date_created,'MM/dd/yyyy') as date_created,  twell.file_number as source_well_file_number,twell.current_well_name as source_well_name, toperator.name as source_well_operator_name,
                  t2.type as water_type_name, t3.common_name as disposal_well_name, t4.name as company_name
...
<table>
    ...
    <tr><td colspan=\"2\" style='height:$td_height;'></td></tr>
    <tr style=\"font-weight:bold;\">
        <td colspan=\"2\">Barrel Rate:</td>        
    </tr>
    <tr>
        <td colspan=\"2\">$".$ticket["barrel_rate"]."</td>        
    </tr>
    <tr><td colspan=\"2\" style='height:$td_height;'></td></tr>
    <tr style=\"font-weight:bold;\">
    ...
    <tr><td colspan=\"2\" style='height:$td_height;'></td></tr>
            ...
            if(count($result_file) > 0)
            {
              ...
              // Aimee Norbee from Hnery Hill asked for the images to appear bigger          
            if($rootScope["SWDCustomer"] == "henryhill") {          
                        if($img_size[1] < 300)
							$ticket_html .= '<img src="../../data/'.$file['filepath'].'" style="height:450px;"/><br/>';
                        else
							$ticket_html .= '<img src="../../data/'.$file['filepath'].'" style="height:450px;"/><br/>'; 
			} else {
					 if($img_size[1] < 300)
				...
				if($rootScope["SWDCustomer"] == "henryhill") {   
                        if($img_size[1] < 300)
                            $ticket_html .= '<img src="../../data/'.$jpg.'" style="height:450px;"/><br/>';
                        else
                            $ticket_html .= '<img src="../../data/'.$jpg.'" style="height:450px;"/><br/>';
                } else {
	                 if($img_size[1] < 300)
