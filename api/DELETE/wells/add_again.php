...
{	   
    ...
    //print_r($_REQUEST);
    $query="insert into wells_list (current_operator_id,current_well_name,township,rng,section,county,field_name, date_created,qq,file_number,api_number,created_by_id)
	...
	$query="select max(id) from wells_list";
    $stmt=pdo_query($pdo,$query,null);
    $row=pdo_fetch_array($stmt);
	$id=$row[0];
    $stmt = pdo_query( $pdo,
					   'select wells_list.id, wells_list.file_number,wells_list.current_well_name,ticket_tracker_operator.name as operator_name, wells_list.api_number, wells_list.file_number, wells_list.qq
                       from wells_list 
