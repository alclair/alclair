...
<div ng-controller="ifi_SerialNumbers">
    <!-- Main Container Starts -->
    <div class="press-enter" style="width:99%;margin-left:10px;">
        <div class="row">
            <div class="col-md-3">
                <a href="<?=$rootScope['RootURL']?>/ifi_uk/orders_uk"><b style="font-size:20px">Serial Numbers</a> (Total: {{TotalRecords}})</b>&nbsp;&nbsp;<b style="display:inline; font-size:20px; color: #007FFF"></b>
            </div>
		...
		<table>		
			...
			<tbody>
				<tr ng-repeat='order in OrdersList'>
					<td  style="text-align:center;" data-title="Category">{{order.category_name}}</td>
					<td  style="text-align:center;" data-title="Product">{{order.product_name}}</td>
					<td  style="text-align:center;" data-title="Serial Number"><a href="<?=$rootScope['RootUrl']?>/ifi_uk/serial_number_history_uk/{{order.serial_number}}">{{order.serial_number}}</a></td>
					<td  style="text-align:center;" data-title="Status">{{order.status}}</td>					
                    <td data-title="Options">
	                    <div style="text-align:center;" >  
							<a class="glyphicon glyphicon-check" style="cursor: pointer;" title="View History" href="<?=$rootScope['RootUrl']?>/ifi_uk/serial_number_history_uk/{{order.serial_number}}"></a>
		                    <!--&nbsp;&nbsp;<button type="button" class="btn btn-primary btn-xs" ng-click="PDF(order.order_id);">Traveler</button>-->
	                        &nbsp;&nbsp;<a class="glyphicon glyphicon-trash" style="cursor: pointer;" title="Delete SN" ng-click="deleteForm(order.id);"></a>
						</div>
                    </td>
				</tr>
			</tbody>
