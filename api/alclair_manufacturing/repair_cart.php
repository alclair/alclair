...
try
{
	...
	$query = pdo_query($pdo, "SELECT * FROM repair_status_table WHERE status_of_repair = 'Repair Cart'", null);
	...
	$stmt = pdo_query( $pdo, 'UPDATE repair_form_active_hp SET repair_status_id = :repair_status_id WHERE id = :id',
		...
		$query = pdo_query($pdo, "SELECT * FROM order_status_table WHERE status_of_order = 'Repair Cart'", null);
		$result = pdo_fetch_array($query);
