...
for ($i = 0; $i < $num_new; $i++) {
	...
	$stmt2 = pdo_query( $pdo, 'SELECT * FROM serial_numbers_uk WHERE serial_number = :serial_number', array(":serial_number"=>$new[$i]));	
...
for ($i = 0; $i < $num_amazon; $i++) { // LOOPING THROUGH ALL OF THE PRODUCTS AND THEIR PREFIXES
	// LOOK FOR THE SERIAL NUMBER TO SEE IF IT ALREADY EXISTS IN THAT WAREHOUSE
	$stmt2 = pdo_query( $pdo, 'SELECT * FROM serial_numbers_uk WHERE serial_number = :serial_number', array(":serial_number"=>$amazon[$i]));	
    $does_SN_exist = pdo_fetch_all($stmt2);
...
for ($i = 0; $i < $num_demo; $i++) { // LOOPING THROUGH ALL OF THE PRODUCTS AND THEIR PREFIXES
	// LOOK FOR THE SERIAL NUMBER TO SEE IF IT ALREADY EXISTS IN THAT WAREHOUSE
	$stmt2 = pdo_query( $pdo, 'SELECT * FROM serial_numbers_uk WHERE serial_number = :serial_number', array(":serial_number"=>$demo[$i]));	
    $does_SN_exist = pdo_fetch_all($stmt2);
...
for ($i = 0; $i < $num_faulty; $i++) {  // LOOPING THROUGH ALL OF THE PRODUCTS AND THEIR PREFIXES
	// LOOK FOR THE SERIAL NUMBER TO SEE IF IT ALREADY EXISTS IN THAT WAREHOUSE
	$stmt2 = pdo_query( $pdo, 'SELECT * FROM serial_numbers_uk WHERE serial_number = :serial_number', array(":serial_number"=>$faulty[$i]));	
    $does_SN_exist = pdo_fetch_all($stmt2);
...
if ( $count_new > 0 ) {   
	...
	$stmt = pdo_query( $pdo, 
					   "INSERT INTO log_movement_uk (movement, movement_type, date, company, warehouse_id, country, name, address_1, city, state, zip_code, ordertype_id, po_number)
					     VALUES (:movement, :movement_type, now(), :company, :warehouse_id, :country, :name, :address_1, :city, :state, :zip_code, :ordertype_id, :po_number) RETURNING id",
	...
	for ($i = 0; $i < $count_new; $i++) {
		$stmt = pdo_query( $pdo, 'SELECT * FROM serial_numbers_uk WHERE serial_number = :sn_new', array(":sn_new"=>$SN_new[$i]) );	
		$result = pdo_fetch_all($stmt);
		if ( count($result) > 0) {
			if ($result[0]["status"] != 'SEALED') {
				$query = "UPDATE serial_numbers_uk SET status = 'SEALED' WHERE serial_number = :serial_number"; 
			...
			$stmt = pdo_query( $pdo, 
					   "INSERT INTO serial_numbers_uk (serial_number, product_id, product_name, category_id, status, active)
		...
		$stmt = pdo_query( $pdo, 
			"INSERT INTO order_tracking_uk (serial_number, date, status, notes, log_id)
...
if ( $count_amazon > 0 ) { 
	...
	for ($i = 0; $i < $count_amazon; $i++) {
		$stmt = pdo_query( $pdo, 'SELECT * FROM serial_numbers_uk WHERE serial_number = :sn_amazon', 	array(":sn_amazon"=>$SN_amazon[$i]) );	
		$result = pdo_fetch_all($stmt);
		if ( count($result) > 0) {
			if ($result[0]["status"] != 'AMAZON') {
				$query = "UPDATE serial_numbers_uk SET status = 'AMAZON' WHERE serial_number = :serial_number"; 
...
if ( $count_demo > 0 ) { 
	...
	for ($i = 0; $i < $count_demo; $i++) {
		$stmt = pdo_query( $pdo, 'SELECT * FROM serial_numbers_uk WHERE serial_number = :sn_demo', array(":sn_demo"=>$SN_demo[$i]) );	
		$result = pdo_fetch_all($stmt);
		if ( count($result) > 0) {
			if ($result[0]["status"] != 'DEMO') {
				$query = "UPDATE serial_numbers_uk SET status = 'DEMO' WHERE serial_number = :serial_number"; 
...
if ( $count_faulty > 0 ) { 
	...
	for ($i = 0; $i < $count_faulty; $i++) {
		$stmt = pdo_query( $pdo, 'SELECT * FROM serial_numbers_uk WHERE serial_number = :sn_faulty', array(":sn_faulty"=>$SN_faulty[$i]) );	
		$result = pdo_fetch_all($stmt);
		if ( count($result) > 0) {
			if ($result[0]["status"] != 'FAULTY') {
				$query = "UPDATE serial_numbers_uk SET status = 'FAULTY' WHERE serial_number = :serial_number"; 
