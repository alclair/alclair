...
{     
	...
	if( !empty($_REQUEST['ship_to_id']) )
    {
        $conditionSql .= " AND t1.id = :ship_to_id";
        $params[":ship_to_id"] = $_REQUEST['ship_to_id'];
    }
    ...
    //Get Total Records
    $query = "SELECT count(t1.id) FROM shipping_addresses AS t1
    ...
    $response["test2"] = $_REQUEST['id'];
        ...
        $query = "SELECT t1.*, to_char(t1.date,'MM/dd/yyyy') as date, to_char(t1.estimated_ship_date,'MM/dd/yyyy') as estimated_ship_date, to_char(t1.received_date,'MM/dd/yyyy') as received_date,IEMs.id AS monitor_id
                  FROM serial_numbers_uk AS t1
