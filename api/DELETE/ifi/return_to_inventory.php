...
if($num_new > 0) {
    $stmt = pdo_query($pdo, "SELECT * FROM order_tracking WHERE serial_number = :serial_number ORDER BY id DESC LIMIT 1", array(":serial_number"=>$new[0]));
    //if ($is_shipped[0]["movement"] == 'EXPORT') { 
    //}
} else if ($num_amazon > 0) {
	 $stmt = pdo_query($pdo, "SELECT * FROM order_tracking WHERE serial_number = :serial_number ORDER BY id DESC LIMIT 1", array(":serial_number"=>$amazon[0]));	
	 
} else if ($num_demo > 0) {
    $stmt = pdo_query($pdo, "SELECT * FROM order_tracking WHERE serial_number = :serial_number ORDER BY id DESC LIMIT 1", array(":serial_number"=>$demo[0]));
} else if ($num_faulty > 0) {
    $stmt = pdo_query($pdo, "SELECT * FROM order_tracking WHERE serial_number = :serial_number ORDER BY id DESC LIMIT 1", array(":serial_number"=>$faulty[0]));	
}

	$check = pdo_fetch_all($stmt);
    $stmt = pdo_query($pdo, "SELECT * FROM log_movement WHERE id = :log_movement_id ORDER BY id DESC LIMIT 1", array(":log_movement_id"=>$check[0]["log_id"]));
    $is_shipped = pdo_fetch_all($stmt);
    
    $company =  $is_shipped[0]["company"];
	$country = $is_shipped[0]["country"];
	$name = $is_shipped[0["name"];
	$address_1 = $is_shipped[0]["address_1"];
	$city = $is_shipped[0]["city"];
	$state = $is_shipped[0]["state"];
	$zip_code = $is_shipped[0]["zip_code"];
